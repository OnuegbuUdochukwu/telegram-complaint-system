...
async def _attempt_request(method: str, url: str, **kwargs) -> Dict[str, Any]:
    ...
    raise (
        ...
        else RuntimeError("Request failed without exception")
    )
    ...
    if BACKEND_URL:
        ...
        try:
            ...
            if not ALLOW_MOCK_FALLBACK:
                raise
            logger.warning(
                "BACKEND_ALLOW_MOCK is enabled; falling back to mock complaint id"
            )
...
async def get_user_complaints(
    telegram_user_id: str, page: int = 1, page_size: int = 20
) -> Dict[str, Any]:
    ...
    if BACKEND_URL and email and password:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.warning(
                    "Service login failed with status %s: %s",
                    resp.status_code,
                    resp.text,
                )
        except Exception as exc:
...
async def upload_photo(
    complaint_id: str, file_bytes: bytes, filename: str, mime_type: Optional[str] = None
) -> Dict[str, Any]:
    ...
    if not token:
        raise RuntimeError(
            "Service token required for photo uploads; set BACKEND_SERVICE_TOKEN."
        )
