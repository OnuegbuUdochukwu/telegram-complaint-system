...
def main():
    ...
    with Session(engine) as session:
        ...
        porter = auth.create_porter(session, full_name="Admin User", password=args.password, email=args.email, role="admin")
        print(f"Created admin porter id={porter.id} email={porter.email}")

    # Use login endpoint to get token
    resp = requests.post("http://127.0.0.1:8001/auth/login", data={"username": args.email, "password": args.password})
