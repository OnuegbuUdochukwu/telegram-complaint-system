...
class TelegramNotifier:
    ...
    def __init__(self):
        ...
        env_chat = os.environ.get("TELEGRAM_ADMIN_CHAT_ID") or config.get(
    ...
    async def send_complaint_alert(self, complaint_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            chat_id = (
                TELEGRAM_ADMIN_CHAT_ID or self.config.admin_chat_ids[0]
                if self.config.admin_chat_ids
                else None
            )
            ...
            logger.info(
                f"Sent complaint alert to Telegram: {complaint_data.get('id', 'unknown')}"
            )
            return True
