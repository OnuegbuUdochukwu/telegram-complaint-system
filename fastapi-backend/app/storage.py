...
if SETTINGS.storage_provider.lower() == "s3":
    try:
        ...
        logger.error("Failed to initialize S3 storage, falling back to local filesystem: %s", exc)
        _s3 = None

logger.info("Storage initialized (provider=%s)", "s3" if _s3 else "local")


def get_s3_key(complaint_id: str, photo_id: str, is_thumbnail: bool = False, content_type: str = "image/jpeg") -> str:
...
def upload_photo(file_data: bytes, complaint_id: str, photo_id: str, 
    ...
    if _s3:
        try:
            ...
            logger.error("Failed to upload photo to S3, falling back to local storage: %s", exc)
