...
async def test_admin_invite_flow(client):
    ...
    with patch("app.main.send_invitation_email", new_callable=AsyncMock) as mock_email, \
         patch("app.main.validate_email", return_value=mock_valid):
        ...
        async with async_session() as session:
            ...
            res = await session.exec(select(AdminInvitation).where(AdminInvitation.email == new_admin_email))
    ...
    with patch("app.main.send_otp_email", new_callable=AsyncMock) as mock_otp_email, \
         patch("app.main.validate_email", return_value=mock_valid):
        ...
        resp = await client.post(
            "/auth/send-otp",
            json={"email": new_admin_email, "purpose": "signup"}
        )
        assert resp.status_code == 200
    ...
    fixed_otp = "123456"
    with patch("app.otp_utils.generate_otp_code", return_value=fixed_otp), \
         patch("app.main.validate_email", return_value=mock_valid):
        
        # Retry send-otp so we know the code
        with patch("app.main.send_otp_email", new_callable=AsyncMock):
            resp = await client.post("/auth/send-otp", json={"email": new_admin_email, "purpose": "signup"})
            assert resp.status_code == 200
    ...
    resp = await client.post("/auth/login", data={"username": new_admin_email, "password": new_password})
    assert resp.status_code == 200
...
async def test_rbac_porter_cannot_invite(client):
    ...
    pwd = "PorterPassword1!"
    async with async_session() as session:
        await auth.create_porter(session, "Porter John", email=email, password=pwd, role="porter")
...
async def test_service_token_auth(client):
    ...
    with patch("app.main.validate_image", return_value=(True, "")) as m_val, \
         patch("app.main.process_image", return_value=(b"opt", b"thumb", 100, 100, "image/jpeg")) as m_proc, \
         patch("app.main.upload_photo", return_value=("http://s3/file.jpg", "http://s3/thumb.jpg")) as m_up:
