...
async def test_get_room_number_invalid(mock_update_context):
    ...
    assert (
        "Invalid room number" in update.message.reply_text.call_args[0][0]
        or "valid room number" in update.message.reply_text.call_args[0][0]
    )
...
async def test_select_severity_and_submit(mock_update_context, monkeypatch):
    ...
    mock_submit = AsyncMock(
        return_value={"complaint_id": "COMP-123", "status": "success"}
    )
