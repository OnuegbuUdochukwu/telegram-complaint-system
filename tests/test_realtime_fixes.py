...
async def admin_token_fixture(async_client: httpx.AsyncClient, session: AsyncSession):
    ...
    if not admin_user:
        ...
        admin_user = await create_porter(
            session,
...
async def porter_token_fixture(async_client: httpx.AsyncClient, session: AsyncSession):
    ...
    if not porter_user:
        ...
        porter_user = await create_porter(
            session,
...
class TestTelegramNotifierFixes:
    ...
    async def test_send_complaint_alert_with_mock(self):
        ...
        if getattr(telegram_notifier, "bot", None) is not None:
            with patch.object(
                telegram_notifier.bot, "send_message", new_callable=AsyncMock
            ) as mock_send:
                complaint_data = {
                    "id": "test-123",
                    "hostel": "John",
...
class TestMainIntegrationFixes:
    ...
    @pytest.mark.asyncio
    async def test_complaint_submission_with_notifications(
        self, session: AsyncSession, admin_token: str, async_client: httpx.AsyncClient
    ):
        ...
        with patch.object(
            telegram_notifier, "send_complaint_alert", new_callable=AsyncMock
        ) as mock_telegram:
            complaint_payload = {
    ...
    @pytest.mark.asyncio
    async def test_status_update_with_notifications(
        self, session: AsyncSession, admin_token: str, async_client: httpx.AsyncClient
    ):
        ...
        with patch.object(
            telegram_notifier, "send_status_update_alert", new_callable=AsyncMock
        ) as mock_telegram:
