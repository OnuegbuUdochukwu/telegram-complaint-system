...
async def test_admin_assigns_another_porter(admin_token, create_and_token):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        r = await client.post(
            f"{BASE}/api/v1/complaints/submit", json=payload, headers=headers
        )
        ...
        r = await client.patch(
            f"{BASE}/api/v1/complaints/{cid}/assign",
            json={"assigned_porter_id": p2_id},
...
@pytest.mark.asyncio
async def test_porter_assigns_self_then_fails_assign_other(
    create_and_token, admin_token
):
    # Fix await
    ...
    payload = {
        "telegram_user_id": "assign-user-2",
        "hostel": "H1",
        "room_number": "A102",
        "category": "test",
        "description": "assign test 2",
        "severity": "low",
    }
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        audit_r = await client.get(
            f"{BASE}/api/v1/complaints/{cid}/assignments", headers=admin_headers
        )
        ...
        found = any(
            a["assigned_to"] == p_id and a["complaint_id"] == cid for a in audits
        )
        ...
        r = await client.patch(
            f"{BASE}/api/v1/complaints/{cid}/assign",
            json={"assigned_porter_id": other_id},
            headers=auth_headers,
        )
