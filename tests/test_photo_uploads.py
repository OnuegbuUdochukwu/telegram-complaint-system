...
def test_upload_photo_requires_auth():
    ...
    upload_resp = httpx.post(
        f"{BASE_URL}/api/v1/complaints/{complaint_id}/photos", files=files, timeout=10.0
    )
    assert (
        upload_resp.status_code == 401
    ), f"Expected 401, got {upload_resp.status_code}"
...
def test_upload_and_list_photos():
    ...
    register_resp = httpx.post(
        f"{BASE_URL}/auth/register", json=register_payload, timeout=10.0
    )
...
def test_list_photos_for_nonexistent_complaint():
    ...
    list_resp = httpx.get(
        f"{BASE_URL}/api/v1/complaints/{fake_id}/photos", headers=headers, timeout=10.0
    )
